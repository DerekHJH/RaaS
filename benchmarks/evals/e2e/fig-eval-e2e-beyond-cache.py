...
def get_y_metric(dataset, approach_name, x_metric):
    dataset = dataset[dataset[f"num_decode_{approach_name}"] > x_metric]
    return np.mean(dataset[f"accuracy_{approach_name}"])
...
if __name__ == "__main__":
        ...
        for j, model_name in enumerate(all_models):
            ...
            for k, approaches in enumerate(all_approaches):
                ...
                for approach_name in approaches:
                    ...
                    y_metric = get_y_metric(dataset, approach_name, x_metric)
    ...
    # Save
    plt.savefig("results/fig-eval-e2e-beyond-cache.pdf", format="pdf", bbox_inches="tight", dpi=400)

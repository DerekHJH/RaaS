...
class EvalEngine:
    ...
    def load_model_for_approach(self, model_name: str, approach_name: str) -> AutoModelForCausalLM:
        ...
        if model_config.model_type == "llama":
            ...
            enable_raas_attention_eval(
                model,
                {
                    "cache_budget": int(approach_name.split("-")[-2]),
    ...
    def test_model(
        ...
        cache_budget = int(self.configs.approach.split("-")[-2])
        alpha = float(self.configs.approach.split("-")[-1])
        past_key_values = RaaSCache(page_size=16, cache_budget=cache_budget, alpha=alpha)

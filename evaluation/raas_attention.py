...
least_recently_used = 16 # Equal to chunk_size
used = []
...
def forward(
    ...
    **kwargs,
    ...
    if q_len > 1 or self.layer_id < 2:
        # Prefill stage 
        if q_len > 1:
            global used, least_recently_used
            used = [least_recently_used for _ in range((hidden_states.shape[-1] + self.chunk_size) / self.chunk_size)]
...
def enable_raas_attention_eval(model, args):

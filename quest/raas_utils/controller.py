from quest.raas_utils.decode_wrapper import BatchDecodeWithPagedKVCacheWrapper
from quest.raas_utils.kv_cache import KvCache
...
class InferenceController:
    def __init__(
        ...
        self.max_kv_pages_num = max_kv_pages_num
        ...
        self.origin_page_budget = page_budget
    ...
    def prepare_metadata(self, seq_len: int):
        ...
        old_pages = len(self.kv_cache.indicies)
        ...
        now_pages = len(self.kv_cache.indicies)

        self.timestamps[:, :, old_pages:now_pages] = self.kv_cache.seqlen # newest timestamp
        self.saved_page_num += appended_new_pages

        if self.saved_page_num > self.origin_page_budget:
            if seq_len > 1:
                logger.warning("Evicing while prefill!")
                self.saved_page_num = self.origin_page_budget
            else:
                assert self.saved_page_num == self.origin_page_budget + 1
                # breakpoint()
                oldest_ind = torch.argmin(self.timestamps[:, :, :now_pages], dim=-1).unsqueeze(-1).long()  # shape: [32, 32, 1]
                pages_ind = torch.gather(self.saved_page_index, dim=-1, index=oldest_ind).long() # shape: [32, 32, 1]
    ...
    def clean_states(self):
        # breakpoint()
        # used by raas: pages that are saved

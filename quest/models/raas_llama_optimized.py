...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
	...
	(seq_len, num_key_value_heads, head_dim) to (seqlen, num_attention_heads, head_dim)
	"""
	seq_len, num_key_value_heads, head_dim = hidden_states.shape
	if n_rep == 1:
		return hidden_states
	hidden_states = hidden_states[:, :, None, :].expand(seq_len, num_key_value_heads, n_rep, head_dim)
	return hidden_states.reshape(seq_len, num_key_value_heads * n_rep, head_dim)
...
class LlamaAttention(nn.Module):
	...
	def forward(
		...
		**kwargs,
		...
		# Hack for GQA: we need to repeat the key and value states to match the number of heads
		...
		# concate attn_output with hidden_states[1:, ...]

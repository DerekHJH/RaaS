...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        torch.cuda.nvtx.range_push(f"input_norm_{self.self_attn.layer_idx}")
        hidden_states = self.input_layernorm(hidden_states)
        torch.cuda.nvtx.range_pop() # input_norm_{self.self_attn.layer_idx}

        torch.cuda.nvtx.range_push(f"Attention_{self.self_attn.layer_idx}")
        # Self Attention
        ...
        torch.cuda.nvtx.range_pop() # Attention_{self.self_attn.layer_idx}
        hidden_states = residual + hidden_states

        # Fully Connected
        residual = hidden_states

        torch.cuda.nvtx.range_push(f"post_attention_layernorm_{self.self_attn.layer_idx}")
        hidden_states = self.post_attention_layernorm(hidden_states)
        torch.cuda.nvtx.range_pop() # post_attention_layernorm_{self.self_attn.layer_idx}

        torch.cuda.nvtx.range_push(f"mlp_{self.self_attn.layer_idx}")
        hidden_states = self.mlp(hidden_states)
        torch.cuda.nvtx.range_pop() # mlp_{self.self_attn.layer_idx}
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        torch.cuda.nvtx.range_pop() # lm_head
        torch.cuda.nvtx.range_pop() # LlamaModel
